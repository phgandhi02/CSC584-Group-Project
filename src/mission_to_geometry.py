...
def adjust_layout_for_mission(layout: dict[str, Any], mission: dict[str, Any], algorithm: str) -> dict[str, Any]:
    """
    ...
    dead_end_count = sum(obj['count'] for obj in objectives if obj['placement_rule'] == 'dead_end')
    needs_large_rooms = any(obj['placement_rule'] in ['central_room', 'checkpoint'] for obj in objectives)
    ...
    if mission_type == 'linear_progression':
        if algorithm in ['random_room_placement', 'bsp', 'hybrid_rooms_caves']:
            ...
            if needs_boss_room:
                ...
                adjusted['target_floor_percent'] = min(0.45, layout.get('target_floor_percent', 0.27) + 0.18)
                ...
                if 'drunkard_params' not in adjusted:
                    adjusted['drunkard_params'] = layout.get('drunkard_params', {}).copy() if 'drunkard_params' in layout else {}
                ...
                adjusted['target_floor_percent'] = min(0.30, layout.get('target_floor_percent', 0.27) + 0.03)
            ...
            params['initial_wall_probability'] = max(0.48, params.get('initial_wall_probability', 0.50) - 0.02)
    ...
    # ===== EXPLORATION =====
        ...
        if algorithm in ['random_room_placement', 'bsp', 'hybrid_rooms_caves']:
            ...
            adjusted['target_floor_percent'] = min(0.30, layout.get('target_floor_percent', 0.27) + 0.03)
            adjusted['straight_bias'] = max(0.60, layout.get('straight_bias', 0.7) - 0.10)
            ...
            params['initial_wall_probability'] = min(0.58, params.get('initial_wall_probability', 0.50) + 0.08)
    ...
    # ===== SURVIVAL =====
        ...
        if algorithm in ['random_room_placement', 'bsp', 'hybrid_rooms_caves']:
            ...
            checkpoint_count = sum(obj['count'] for obj in objectives if obj['placement_rule'] == 'checkpoint')
            adjusted['max_rooms'] = max(8, min(20, layout.get('max_rooms', 12) + checkpoint_count))
            adjusted['room_size_min'] = max(5, layout.get('room_size_min', 4) + 1)
        
        elif algorithm == 'drunkards_walk':
            # Tighter passages for tension
            adjusted['target_floor_percent'] = max(0.22, layout.get('target_floor_percent', 0.27) - 0.05)
            adjusted['straight_bias'] = min(0.85, layout.get('straight_bias', 0.7) + 0.15)
            ...
            if 'cellular_params' not in adjusted:
                adjusted['cellular_params'] = layout.get('cellular_params', {}).copy()
            params = adjusted['cellular_params']
            params['initial_wall_probability'] = min(0.55, params.get('initial_wall_probability', 0.50) + 0.05)
            params['iterations'] = params.get('iterations', 4)
    
    # ===== KEY HUNT =====
    elif mission_type == 'key_hunt':
        key_count = sum(obj['count'] for obj in objectives if obj['objective_type'] == 'key')
        ...
        if algorithm in ['random_room_placement', 'bsp', 'hybrid_rooms_caves']:
            ...
            if 'cellular_params' not in adjusted:
                adjusted['cellular_params'] = layout.get('cellular_params', {}).copy()
            params = adjusted['cellular_params']
            params['initial_wall_probability'] = min(0.54, params.get('initial_wall_probability', 0.50) + 0.04)
            params['iterations'] = params.get('iterations', 4)
    ...
    # ===== MULTI OBJECTIVE =====
        ...
        if algorithm in ['random_room_placement', 'bsp', 'hybrid_rooms_caves']:
            ...
            if 'cellular_params' not in adjusted:
                adjusted['cellular_params'] = layout.get('cellular_params', {}).copy()
            params = adjusted['cellular_params']
            params['initial_wall_probability'] = min(0.53, params.get('initial_wall_probability', 0.50) + 0.03)
            params['iterations'] = params.get('iterations', 4)
    ...
    # ===== ENSURE DEAD ENDS =====
    if needs_dead_ends and algorithm in ['random_room_placement', 'bsp', 'hybrid_rooms_caves']:
        # More rooms = more potential dead ends
        adjusted['max_rooms'] = max(adjusted.get('max_rooms', 15), 12 + dead_end_count)
    
    # ===== ENSURE LARGE ROOMS =====
    if needs_large_rooms and algorithm in ['random_room_placement', 'bsp', 'hybrid_rooms_caves']:
...
def validate_mission_feasibility(layout: dict[str, Any], mission: dict[str, Any]) -> list[str]:
    """
    ...
    dead_end_needed = sum(obj['count'] for obj in objectives if obj['placement_rule'] == 'dead_end')
    
    # Check room-based algorithms
    max_rooms = layout.get('max_rooms', 15)
    if max_rooms < total_objectives:
        warnings.append(f"Only {max_rooms} rooms but {total_objectives} objectives - may not all place")
    
    if dead_end_needed > max_rooms // 2:
        warnings.append(f"Need {dead_end_needed} dead-ends but only {max_rooms} rooms - may be insufficient")
